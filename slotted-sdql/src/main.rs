...
fn main() {
    ...
    may_trace_optimize(filename, &prog_str);
}

fn optimize(filename: &str, prog_str: &str) {
  let init_span = tracing::trace_span!("init").entered();
  ...
  let extract_span = tracing::trace_span!("extract").entered();
  let cost_func = SdqlCost { egraph: &eg };
  let extractor = Extractor::<_, SdqlCost>::new(&eg, cost_func);
...
#[cfg(feature = "trace")]
fn may_trace_optimize(filename: &str, prog_str: &str) {
    ...
    span.in_scope(|| {
        optimize(filename, prog_str);
    });
    tracing::trace!(name: "display", ""); // trigger display of stats
}
